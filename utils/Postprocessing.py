...
def postprocessing(res_df, df, sets_df, defaults_df):
    ...
    # Step 1: Check if ResidualCapacity exists in df
    if 'ResidualCapacity' in df['PARAM'].values:
        rescap = df[df['PARAM'] == 'ResidualCapacity'][['TECHNOLOGY', 'YEAR', 'VALUE', 'REGION']].copy()
        rescap['YEAR'] = rescap['YEAR'].astype(float)
    else:
        # Create an empty DataFrame with required structure if ResidualCapacity is missing
        rescap = pd.DataFrame(columns=['TECHNOLOGY', 'YEAR', 'VALUE', 'REGION'])

    # Step 2: Create full Cartesian product for years and technologies
    all_years = pd.DataFrame({'YEAR': sets_df['YEAR'].astype(float)})
    techs = sets_df['TECHNOLOGY'].unique()  # Use all technologies from sets_df
    region = sets_df['REGION'].iloc[0]
    ...
    for tech in techs:
        ...
        tech_df['REGION'] = region

        # Merge and fill missing values
        if not rescap.empty and tech in rescap['TECHNOLOGY'].values:
            merged_df = pd.merge(tech_df, rescap[rescap['TECHNOLOGY'] == tech], how='left', on=['YEAR', 'TECHNOLOGY', 'REGION'])
        else:
            # If technology is not in rescap, set VALUE to 0 for all years
            tech_df['VALUE'] = 0
            merged_df = tech_df

        merged_df['VALUE'] = merged_df['VALUE'].fillna(0)
        full_rescap.append(merged_df)

    # Combine all technology DataFrames
    rescap = pd.concat(full_rescap, ignore_index=True)

    # Step 3: Handle missing technologies in sets_df
    missing_techs = set(sets_df['TECHNOLOGY']) - set(rescap['TECHNOLOGY'])
    missing_entries = []

    for tech in missing_techs:
        default_value = defaults_df.loc[defaults_df['PARAM'] == 'ResidualCapacity', 'VALUE'].item()
        for year in sets_df['YEAR']:
            missing_entries.append({'TECHNOLOGY': tech, 'YEAR': float(year), 'VALUE': default_value, 'REGION': region})

    # Append missing entries
    if missing_entries:
        rescap_new = pd.DataFrame(missing_entries)
        rescap = pd.concat([rescap, rescap_new], ignore_index=True)

    # Save to Excel
    rescap.to_excel('rescapupdated.xlsx', index=False)
